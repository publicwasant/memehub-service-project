...
router.post('/', JWTToken.middle(), function (req, res) {
    ...
    const input = {user_id: body.user_id, comment_id: body.comment_id};
    const output = fg.output;

    const emitter = new events.EventEmitter();

    emitter.on('delete', function (_attach_) {
        const sql = "DELETE FROM comments WHERE c_user_id=? AND c_id=?";

        database.query(sql, [input.user_id, input.comment_id], function (err, result) {
            if (err) {
            ...
            if (deleted != 0) {
                return emitter.emit('response', {
                    status: 1,
                    descript: 'ลบความคิดเห็นสำเร็จแล้ว',
                    data: {
                        deleted: deleted
                    }
                });
                ...
                return emitter.emit('response', {
                    status: 0,
                    descript: 'ลบความคิดเห็นไม่สำเร็จ!'
                });
            }
        });
    });
