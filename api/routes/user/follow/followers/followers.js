...
router.get('/', function (req, res) {
    ...
    if (find.page == 'example') {
        return res.end(fs.readFileSync(__dirname+'/fg_followers.json'));
    }

    return fs.readFile(__dirname+'/followers.html', function (err, data) {
        if (!err) {
            return res.end(data);    
        }
...
router.post('/', JWTToken.middle(), function (req, res) {
    ...
    const fg = JSON.parse(fs.readFileSync(__dirname+'/fg_followers.json'));
    const body = req.body;
    ...
    emitter.on('select', function (_attach_) {
        const sql = "SELECT f_target_id FROM follows WHERE f_follower_id=?";
    ...
    emitter.on('compress', function (_attach_) {
        ...
        const users_id = function (_raw_) {
            ...
            for (var i = 0; i < _raw_.length; i++) {
                _temp_.push(_raw_[i].f_target_id);
            }
